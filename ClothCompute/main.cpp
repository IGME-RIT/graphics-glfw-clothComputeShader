...
void init()
{
	...
	std::string computeShader = readShader("../Assets/ComputeShader.glsl");
...
void update()
{
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		// if readBuffer is 0, then 1 - 0 = 1, swapping from 0 to 1
		// if readBuffer is 1, then 1 - 1 = 0, swapping from 1 to 0

		//Bind the appropriate buffers, to the appropriate locations
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, posBuf[readBuffer]);		// read positions
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, posBuf[1 - readBuffer]);	// write positions
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, velBuf[readBuffer]);		// read velocity
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, velBuf[1 - readBuffer]);	// write velocity
	}
}
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Cloth simulation -- Hold and release Spacebar", nullptr, nullptr);
